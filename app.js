...
function safeSelect(form,name,value){try{form.getRadioGroup(name).select(value)}catch(e){try{form.getDropdown(name).select(value)}catch(_){console.debug('Groupe absent',name,value)}}}
function companyText(){const s=settings();return [s.entrepriseNom,s.entrepriseAdresse,`Tél : ${s.entrepriseTel}`,s.entrepriseEmail].filter(Boolean).join('\n')}
...
async function createPdf(){if(!$('#form').reportValidity())return;save();const d=data(),cfg=FIELD_MAP[d.type];const response=await fetch(cfg.template,{cache:'no-store'});if(!response.ok)throw new Error(`Le modèle PDF ${cfg.template} est introuvable sur GitHub (${response.status}). Téléversez tous les fichiers de la version complète.`);const bytes=await response.arrayBuffer();const head=new Uint8Array(bytes.slice(0,5));if(String.fromCharCode(...head)!=='%PDF-')throw new Error(`Le fichier ${cfg.template} reçu n'est pas un PDF valide. Rechargez la version complète sur GitHub.`);const doc=await PDFDocument.load(bytes),formPdf=doc.getForm(),f=cfg.fields;
 ...
 const out=await doc.save();const blob=new Blob([out],{type:'application/pdf'}),url=URL.createObjectURL(blob),name=`ATTESTATION-${d.type.toUpperCase()}-${(d.clientNom||'CLIENT').replace(/[^a-z0-9]/gi,'_')}-${d.dateVisite||''}.pdf`;if(navigator.share&&navigator.canShare){try{const file=new File([blob],name,{type:'application/pdf'});if(navigator.canShare({files:[file]})){await navigator.share({files:[file],title:name});return}}catch(e){}}
